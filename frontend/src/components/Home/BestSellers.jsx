...
const API_BASE = import.meta.env.VITE_API_BASE_URL; // ✅ use your live backend URL
...
const BestSellers = () => {
  ...
  useEffect(() => {
    ...
    const fetchProducts = async () => {
      try {
        ...
        if (isMounted) setToast({ message: error.message || "Error loading products", type: "error" });
      } finally {
  ...
  return (
    <section ref={ref} className={`best-sellers ${inView ? "active" : ""}`}>
      ...
      {loading ? (
        ...
        <div className="product-list">
          {products.map((product, index) => {
            let parsedImages = [];
            try { parsedImages = JSON.parse(product.image_url); } catch { parsedImages = []; }
            ...
            return (
              <div
                ...
                role="button"
                tabIndex={0}
                onKeyDown={(e) => { if (e.key === "Enter" || e.key === " ") handleViewProduct(product); }}
                aria-label={`View details for ${product.name}`}
              >
                <img src={firstImage} alt={product.name} className="bestseller-img" loading="lazy" />
                <h3>{product.name}</h3>
                <div className="feature-price">
                  <span className="current-price">${parseFloat(product.price).toFixed(2)}</span>
                  {product.discount_price && <span className="old-price">${parseFloat(product.discount_price).toFixed(2)}</span>}
                </div>
                <button onClick={(e) => { e.stopPropagation(); handleAddToCart(product); }} aria-label={`Add ${product.name} to cart`}>
                  Add to Cart
                </button>
              </div>
            );
      ...
      {toast && <Toast message={toast.message} type={toast.type} actions={toast.actions} onClose={() => setToast(null)} />}
    </section>
  );
};
